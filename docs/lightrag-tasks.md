...
| RFC-I06 | Phase 2 | 抽象 `CatalogProvider`，保留 single-worker local provider，实现首个 PostgreSQL shared provider | RFC-I03、RFC-I00 catalog 决策 | 已完成 | revision/CAS、分页、唯一约束、cache invalidation；local+workers>1 fail startup |
| RFC-I07 | Phase 2 | 实现 catalog lifecycle、幂等 management operation、fencing 与 tombstone | RFC-I06、RFC-I04 | 已完成 | CREATING/MIGRATING/ACTIVE/DELETING/TOMBSTONED/ERROR 可恢复；kill owner 与 stale commit 测试通过 |
| RFC-I08 | Phase 3 | 重构为 explicit `WorkspaceExecutionContext` 与 fail-closed ContextVar adapter，修正 selector/response contract | RFC-I06、RFC-I07 | 已完成 | absent/empty/invalid/unknown/inactive 全矩阵通过；缺 context typed failure；成功响应暴露 resolved ID |
| RFC-I09 | Phase 3 | 实现 per-worker lease pool：entry state、single-flight、resource weight、safe LRU、backoff、503 背压 | RFC-I08 | 已完成 | stream/background lease 阻止 eviction/delete；满池无安全 victim 返回 503；取消 exactly-once release |
| RFC-I10 | Phase 3 | 拆分 side-effect-free `/health`、`/ready`、catalog/pool peek；移除 first-access migration | RFC-I07、RFC-I09、RFC-I02 | 已完成 | health/ready 产生零实例构造、零 storage init、零 migration；runtime observation 可返回 UNLOADED |
| RFC-I11 | Phase 4 | 建立 control-plane migration/recovery coordinator，分页枚举全 catalog 并使用 lease/fencing | RFC-I07、RFC-I10 | 已完成 | full restart 无需用户访问即可恢复全部 ACTIVE workspace；一个坏库不阻塞其他库；migration 非 request-owned |
| RFC-I12 | Phase 4 | 把 request/stream/background/pipeline/delete 全部绑定 explicit context 和 lease handoff，接入现有 ingress/fence | RFC-I08、RFC-I09、RFC-I11 | 已完成 | background handoff 原子；相同 doc hash 跨库状态全生命周期不串；worker kill 后旧 owner 不能提交 |
| RFC-I13 | Phase 4 | 实现 fenced 两阶段删除和 durable cleanup journal | RFC-I04、RFC-I07、RFC-I09、RFC-I12 | 已完成 | ACTIVE→DELETING 拒绝新 lease；partial drop 可续跑；descriptor mismatch 禁止 destructive action；最终 tombstone |
...
| RFC-I16 | Phase 5 | 将 same-host Manager 能力封装为 coordinator provider，完成 Gunicorn support matrix | RFC-I06、RFC-I11、RFC-I15 | 已完成 | 任意 worker 路由、catalog revision、worker kill、provider cap 和 pipeline owner 测试通过；无 sticky session |
...
| RFC-I19 | Phase 7+ | 按 backend 分拆 strict physical profile 的 provision/migration/delete/backup 硬化 | RFC-I04、RFC-I07、RFC-I13 | 已完成 | 每个 backend 独立 PR、真实服务 integration、resource ownership 与恢复文档，不混入 core PR |
...
### 2026-08-03T20:32:16+08:00 — RFC-I06～I07 Phase 2 durable catalog 与生命周期状态机

- 代码基线与提交：基于 `dev@d33b3595` 实施，形成 `638a778d feat(workspace): add durable catalog lifecycle`；未跟踪的 `.codex/` 未修改、未暂存、未提交。
- Catalog provider：新增 async `CatalogProvider` contract、single-worker `LocalCatalogProvider` 和首个 shared `PostgresCatalogProvider`。PostgreSQL schema 包含 catalog/operation 表、唯一 canonical/effective workspace、唯一非空 physical profile、revision CAS、确定性分页、operation 索引和单调 fencing sequence；provider 每次读写更新带 revision 的本地 cache，而数据库仍是唯一真相。
- Lifecycle：record 增加 `CREATING/MIGRATING/ACTIVE/DELETING/TOMBSTONED/ERROR`、revision、schema version、current operation 和脱敏错误字段；create/delete 原子建立 durable operation，支持 `Idempotency-Key` 重放与 payload 冲突检测。owner claim 获得 fencing token，record transition 和 terminal operation commit 同时校验 revision/state/owner/token；旧 owner/stale revision 无法提交。
- API 与安全：create/delete 默认返回 `202 + operation_id + Location`，`Prefer: wait=N` 在最大 10 秒（可配置、硬上限 60 秒）内等待并返回终态；新增 operation 查询。multi mode 启动必须配置独立 `LIGHTRAG_ADMIN_API_KEY`，create/update/delete 同时要求普通认证和 `X-LightRAG-Admin-Key`；list/get/operation read 保持普通认证。只有 ACTIVE record 可进入数据面，初始化/迁移失败为 ERROR 且不会 first-access 发布。
- 删除语义：删除先在 manager 内阻断新 request lease并检查 active request/pipeline，再原子 ACTIVE→DELETING；cleanup owner 在 descriptor pre-delete gate 后 drop 12 类 storage、finalize、删除受控 input 子目录并提交 durable tombstone。失败进入 ERROR，operation 保留 FAILED，可由相同幂等键重新 claim；最终的逐 backend cleanup journal 仍属于 RFC-I13。
- 单元与故障验证：provider/API/config/endpoint 聚焦回归 `48 passed`；覆盖 create/delete 幂等重放、payload 冲突、revision CAS、旧 owner fencing、retry token 单调递增、catalog reload/pagination、PostgreSQL SQL CAS guard、Admin key、202/Prefer wait、失败迁移永不进入数据面和 durable tombstone。
- 扩展 API 回归：排除 Windows 不可导入的 POSIX Gunicorn orphan 文件后为 `715 passed, 14 skipped, 3 failed`；三个失败仍是既知 Windows 缺 `fcntl` 以及两项既有 API-key-only 401/403 断言差异，与 Phase 0～1 记录一致。全仓 pre-commit 二次运行全部通过；三个 multi-tenant Compose 使用各自 example env 完成 `docker compose config --quiet`。
- 外部验证边界：本机 Docker Engine 仍未运行，故未把 PostgreSQL provider 的事务假件测试声明为真实数据库集成通过。真实 PostgreSQL 事务/并发 owner kill、跨进程 cache consistency 将在 Docker 可用和 RFC-I16 Gate 前补做；当前 support matrix 仍只允许 multi mode 单 worker，PostgreSQL catalog 可用但不能据此宣称 Gunicorn 安全。
- 兼容与状态：默认 mode 仍为 legacy，local version-1 JSON 以 additive 字段加载旧 records；默认 namespace 不搬迁。部署示例新增独立 Admin key，文档示例适配嵌套 lifecycle response。RFC-I06、RFC-I07 完成；下一项为 RFC-I08 explicit `WorkspaceExecutionContext` 与 selector contract。

### 2026-08-03T20:57:31+08:00 — RFC-I08～I10 Phase 3 lease pool 与只读路由

- 代码基线与提交：基于 `dev@ed145afb` 实施，形成 `09d61a07 feat(workspace): add leased request routing`；未跟踪的 `.codex/` 未修改、未暂存、未提交。
- Explicit context：新增 frozen `WorkspaceExecutionContext`，把 catalog record、immutable binding、RAG 与 DocumentManager 作为一次 lease 的完整 authority snapshot；多 workspace proxy 缺少 ContextVar 时 typed fail-closed，仅为 Ollama 启动期 metadata 保留显式 allowlist，不再静默回落 default。
- Selector 与响应契约：只有 header 缺失选择 default；present-empty、whitespace、illegal、unknown、CREATING/MIGRATING/ERROR、DELETING、TOMBSTONED 分别按 400/404/409/503 契约拒绝。成功数据面响应回显 canonical `LIGHTRAG-KNOWLEDGE-BASE` 并合并 `Vary`；health 不解释 selector，也不发布该参数。
- Lease pool：新增 per-worker `WorkspaceInstancePool`，包含 INITIALIZING/READY/DRAINING/FINALIZING/FAILED、keyed single-flight、foreground/stream/background exactly-once lease、默认库 pin、entry/weight 双预算、idle LRU、安全 pipeline 检查、指数 backoff 和 failed-cleanup quarantine。满池且没有安全 victim 返回 retryable 503，而不是业务 409；stream/background lease 与 delete reservation 原子互斥。
- 无副作用观测：新增 `/ready` 的 catalog/pool peek 与 lifecycle counter invariant；`/health` 只读启动时 default snapshot，不再调用 `get_context`；`/documents/pipeline_status` 对未加载 ACTIVE workspace 返回 `runtime_state=UNLOADED`，不会构造 RAG、初始化 storage 或迁移。数据面首次 load 只执行 `initialize_storages()`，migration 已从 request path 移除。
- Phase Gate：server 默认 `LIGHTRAG_ENABLE_NON_DEFAULT_WRITES=false`；endpoint policy 识别的非默认 DATA_WRITE 在实例构造前返回 non-retryable 503。query/read 已可使用 lease routing，但 ingestion/mutation 必须等 RFC-I11～I15 的 recovery、handoff 和 shared admission 全部完成后才打开。pool 容量、weight 与失败 backoff 均已加入 `env.example`。
- 确定性验证：Phase 3 pool、selector、stream、health/ready、catalog 与 deployment 聚焦 suite `84 passed`；格式化后的核心回归 `62 passed`；覆盖首次构造 barrier、lease exactly-once、stream 生命周期、background/delete 互斥、weight 背压、LRU、revision refresh、failure backoff、first-access no-migration、UNLOADED observation 和 response header/Vary。
- 扩展 API 回归：显式排除三个已记录 Windows 基线节点后为 `732 passed, 14 skipped, 3 deselected`，新增失败为 0。三个 deselected 仍是 POSIX Gunicorn `fcntl` 不可导入，以及两项既有 API-key-only 401/403 断言差异；全仓 pre-commit 首次格式化 1 个文件，第二次所有 hook 通过，非本阶段文件的机械格式变化已恢复且未提交。
- 兼容、回滚与残余边界：legacy mode、default namespace 与现有数据布局不变；read-only/query 路由可通过关闭 multi mode 整体回滚，pool 是进程本地缓存而非 catalog authority。真实 PostgreSQL catalog、多进程 owner kill、跨 workspace restart recovery、durable per-resource cleanup、service-global provider cap、公平调度和 Gunicorn 仍未在本机外部服务验证，不能据此宣称多 worker 或 multi-node 安全。
- 状态：RFC-I08、RFC-I09、RFC-I10 完成；下一项为 RFC-I11 control-plane migration/recovery coordinator。Phase 4 完成前继续保持 non-default write gate，Phase 5 shared admission 完成前不解除生产写入限制。

### 2026-08-03T21:23:41+08:00 — RFC-I11～I13 Phase 4 fenced recovery 与完整后台上下文

- 代码基线与提交：基于 `dev@f1b4c95a` 实施，形成 `9f2cba61 feat(workspace): add fenced startup recovery`；未跟踪的 `.codex/` 与测试临时目录未暂存、未提交。
- Control plane recovery：新增 `WorkspaceRecoveryCoordinator`，启动期先用新 fencing token 接管 PENDING/RUNNING/FAILED 的 CREATE/MIGRATE/DELETE operation，再以稳定 operation-ID 游标和 catalog record 游标分页恢复全量记录。每个 ACTIVE workspace 原子进入 MIGRATING，使用独立 control context 完成 storage migration 和 PENDING/PROCESSING pipeline recovery，再 fenced 发布 ACTIVE；单个坏 workspace 进入 ERROR，但不会阻塞后续分页项或默认库启动结论。
- 显式后台 handoff：新增 framework-neutral workspace-scope hook；request 在创建 asyncio child 前原子取得同一 immutable context 的 background lease，child 重绑 ContextVar，并在成功、异常、取消、任务创建失败或 reservation refusal 时 exactly-once 释放。父 request 结束后后台 lease 仍阻止 pool eviction 和 knowledge-base deletion，pipeline 派生任务继承显式 authority，不回退 default。
- Durable cleanup：DELETE operation metadata 记录 12 个 storage role 与受控 input directory 的逐步 `cleanup_completed` journal。每一步 checkpoint 都验证 owner/fencing token；失败转 ERROR/FAILED 并保留进度，重试使用更大的 token，仅续跑未完成资源，全部完成后才写 `cleanup_complete`、TOMBSTONED 与 SUCCEEDED。pre-delete descriptor gate 仍先于任何 drop，旧 owner 无法写 checkpoint 或终态。
- doc-status 隔离：新增默认 JSON doc-status E2E，两个 named workspace 使用完全相同 doc ID/content hash，依次验证 PENDING、PARSING、ANALYZING、PROCESSING、PROCESSED、FAILED、retry、进程级 shared-state 重启与单边 delete；另一 workspace 的 status、track/metadata 与 hash 始终保持不变。
- 严格验证：格式化后 Phase 4 聚焦 suite `74 passed`；API 全量相关回归 `739 passed, 14 skipped, 3 deselected`；pipeline 与两类 managed background launcher 全量 `632 passed`；全仓 pre-commit 二次运行所有 hook 通过。首次 pipeline sweep 的 6 项失败由 Windows 长临时路径导致原子临时文件创建失败，切换到短临时根目录后对应文件 `26 passed` 且全量 `632 passed`，未作为业务失败掩盖。
- 已知环境与支持边界：三个 API deselection 仍为 Windows 无 POSIX `fcntl` 的 Gunicorn 测试，以及两项既有 API-key-only 401/403 契约差异；tiktoken 测试使用本机已有 cache，未联网。真实 PostgreSQL catalog transaction、owner kill、Gunicorn worker 协调和外部存储 restart recovery 尚未在 Docker 服务上验证，不能宣称 multi-worker/multi-node 安全。
- 兼容、回滚与 Gate：legacy/default 数据布局不变；库外直接调用 managed task launcher 时 workspace hook 是 no-op。恢复页大小由 `LIGHTRAG_RECOVERY_PAGE_SIZE` 控制。当前 support matrix 仍只允许 multi/local/local/single-worker；RFC-I14～I16 未完成前 `LIGHTRAG_ENABLE_NON_DEFAULT_WRITES=false` 继续作为默认安全门禁。状态：RFC-I11、RFC-I12、RFC-I13 完成；下一项为 RFC-I14 service-level provider admission。

### 2026-08-03T21:56:27+08:00 — RFC-I14～I16 Phase 5 service-level admission 与同机协调器

- 代码基线与提交：基于 `dev@54ccfc8e` 实施，形成 `b0982bb6 feat(workspace): add service-wide fair admission` 与 `f735e18b feat(workspace): open verified single-worker writes`；未跟踪的 `.codex/` 与测试临时目录未暂存、未提交。
- 统一资源预算：新增 app-scoped `ResourceAdmissionController`；每个知识库实例不再为 LLM 各角色、embedding 和 rerank 叠加独立 deployment budget。provider 原始调用统一经 controller 准入，原有 per-instance priority queue 保留本地排队/超时语义但不重复获取 global slot。pipeline 同样进入 deployment-total active cap。
- 公平与过载：调度以 public knowledge-base ID 为公平单位，使用 cost-bounded DRR；query、management、ingestion、recovery 使用优先级并由 aging 防止低优先级永久饥饿。全局与单 workspace pending 队列均有硬上限，总等待有 deadline；饱和与超时分别返回稳定 429/503、`Retry-After` 和无 secret 的错误码。health/ready 暴露有界聚合快照，不加载实例。
- 显式 authority：request 把 immutable workspace ID 与 operation kind 绑定到 `WorkspaceExecutionScope`；provider、pipeline 和 lifecycle recovery 校验 scope 与实例固定 binding，一旦错配 fail closed。管理/恢复派生 task 主动重绑目标 workspace，避免继承父管理请求的 selector。
- 同机协调：新增 local/Manager `WorkspaceCoordinator` provider。Gunicorn 多 worker 配置必须同时选择 PostgreSQL catalog 与 Manager coordinator；startup recovery 由 master-owned tracked slot 唯一授权，持续 heartbeat，shared state 使用递增 fencing token，follower 复用终态，死亡 owner 的 lease 被回收后可由新 worker 接管。每个 forked worker 重新生成 lifecycle owner ID，避免继承 preload master 身份。
- 写入 Gate：RFC-I11～I15 的依赖已满足后，single-worker multi mode 在未显式配置时默认允许非默认知识库写入；可用 `LIGHTRAG_ENABLE_NON_DEFAULT_WRITES=false` 回滚。同机 multi-worker 仍默认关闭，只有显式设置 true 才 opt-in。
- 严格测试：新增 admission/coordinator/deployment 确定性测试，覆盖 LLM/embedding/rerank peak、pipeline cap、DRR、aging、bounded queue、429 contract、两 controller 共享 Manager slot、startup single-owner、shared failure、dead PID reclaim 与 fence 递增。聚焦生命周期/准入 `69 passed`；目录、pool、catalog、health、provider queue/global slot `124 passed`；pipeline、dead reservation 与 ingress sweep 为 `676 passed, 1 skipped`，skip 是 Windows 无 POSIX `SIGKILL`。
...
### 2026-08-05T12:02:14+08:00 — RFC-I16/I19 真实外部门禁与最终容器交付

- 代码基线与分类提交：基于 `dev@4d0335d9` 实施，推送前 rebase 到包含上游 BFS follow-up 的 `origin/dev@19e9e68e`，最终形成 `e7d053f6 fix(storage): honor instance physical profiles`、`e2f0f02f fix(api): harden catalog and worker startup`、`4c2b3545 test(workspace): add real physical isolation gates` 与 `9c57778f build(docker): optimize multi-worker integration image`；`.codex/` 始终未暂存、未提交。
- 真实环境发现并修复：实例传入完整 physical `storage_profile` 时，`LightRAG.__post_init__` 不再被同 backend 的进程级环境变量预检查误拒绝；profile 缺必填字段仍 fail closed，未提供 profile 时保持 legacy env 契约。PostgreSQL catalog 把 JSON-safe ISO 时间转换为 asyncpg 所需的 timezone-aware `datetime`，并消除 lifecycle SQL 中 `$4` 的 varchar/text 类型推断冲突。Gunicorn post-fork logger 遍历使用 registry snapshot，保留 `uvicorn.error` 可见性，初始化失败打印 traceback 后重新抛出。
- RFC-I16 外部门禁：使用 `docker-compose.integration-gunicorn.yml`、PostgreSQL catalog、Manager coordinator 与 2 个 Linux Gunicorn worker，在干净卷执行 create、重复无 sticky-session route、catalog rename/revision、delete/cleanup。最终镜像的知识库 `kb_a0ec079b5972` 创建与删除均 `SUCCEEDED`，6 次 `/documents/status_counts` 均回显正确 `LIGHTRAG-KNOWLEDGE-BASE`，删除为 `TOMBSTONED` 且 `cleanup_complete=true`。worker PID 69 被 `SIGKILL` 后由 PID 768 自动替换，期间 `/health` 保持 healthy；RFC-I16 更新为已完成，但 multi-node 仍不在该结论内。
- RFC-I19 主组合：`docker-compose.integration-physical.yml` 启动两套 PostgreSQL、Neo4j、Redis；`LIGHTRAG_PHYSICAL_IT=true` 的四 storage family 双 profile 测试为 `1 passed`。同逻辑 ID 在 A/B 均可写读，drop A 后 B 的 KV/vector/graph/doc-status 完整。备份 seed/verify 各 `1 passed`；在同一恢复点用 `pg_dump/pg_restore`、Redis RDB、`neo4j-admin database dump/load` 恢复 PostgreSQL catalog 和四类 storage 数据。
- RFC-I19 扩展后端：`docker-compose.integration-physical-extended.yml` 的 qdrant、memgraph、mongo、opensearch、milvus profile 分别启动双 endpoint；逐项设置 `LIGHTRAG_EXTENDED_PHYSICAL_IT=<backend>` 运行 `tests/workspace/test_physical_services_multitenant_integration.py -k extended --run-integration`，五类结果均为 `1 passed, 4 skipped`（Milvus 另有 2 个非目标 backup case deselect）。每项覆盖 initialize/migration、相同 ID 写读、drop A 与 B 不受影响；Mongo/OpenSearch 的异步 vector index 使用有界轮询而非固定长 sleep。
- 回归与平台边界：profile/catalog/Gunicorn 聚焦 `59 passed`；`tests/kg` 最终 `1507 passed, 151 skipped`，`tests/workspace` `75 passed, 7 skipped`。未过滤 API sweep 为 `775 passed, 14 skipped, 3 failed`；三个仍是 Windows 无 `fcntl` 与两个既有 API-key-only 401/403 断言差异，本轮新增失败为 0。显式排除这三个已知环境/基线节点的 API 绿色门禁为 `775 passed, 14 skipped, 3 deselected`。Windows process tests 仅在具备 POSIX `SIGKILL/os.kill(pid, 0)` 时运行，源码架构扫描显式使用 UTF-8；聚焦为 `18 passed, 6 skipped`。rebase 后针对远端变化重跑 MongoDB/Neo4j/OpenSearch 为 `475 passed, 6 skipped`，workspace + 本轮聚焦为 `134 passed, 7 skipped`。最终 pre-commit 五个 hook 全通过。
- Docker 交付：Dockerfile 把 lock 依赖、前端、tiktoken/spaCy cache 与源码分层，final stage 直接复用 builder venv；`COPY --chown` 取代约 187.6 秒的递归 ownership rewrite，稳定 apt/user 层前移，entrypoint 在镜像内去 CRLF，`.dockerignore` 排除本地 `node_modules`。移除递归 chown 后构建 `50.6s`、全缓存构建 `8.8s`；补充 `/app/lightrag.log` 目录权限后的最终构建 `53.2s`，且完成上述干净卷烟测。后续两次纯缓存复验在项目层前分别被 Docker Hub `EOF` 与 GHCR TLS timeout 阻断，按外部 registry 事件记录。三份 Compose 的 `docker compose config --quiet` 均通过。
- 清理、兼容与下一步：所有 integration 容器、网络、测试卷、仓库内 `it_*` 临时目录和本轮临时备份副本均已删除；默认 legacy/layout、logical record 与 operator-owned 外部服务均未搬迁或销毁。RFC-I16、RFC-I19 当前为已完成；RFC-I20 external coordinator/multi-node 仍未开始，不宣称网络分区、跨节点 lease/admission 或 multi-node production safety。

### 2026-08-05T19:06:40+08:00 — upstream/main 双向同步与合并回归

- Git 基线：获取 `upstream/main@29c089ff`，先将 fork `origin/main@0e60c568` 快进到本地 `main`，再形成 `252db390 Merge upstream/main into main`；随后在 `dev` 形成 `24b4d70f Merge main into dev`。最终步骤是在本轮修复、验证与提交完成后把 `dev` 合并回 `main`，并显式推送 fork 的两个分支。
- 冲突处理：`lightrag/api/lightrag_server.py` 保留多知识库 `build_rag` factory、实例级 input/profile 绑定，同时吸收上游 embedding overlap 配置；`lightrag/kg/postgres_impl.py` 同时保留显式 PGVector 标识与实例级 physical profile；`Dockerfile` 保留分层、`COPY --chown` 和非 root 运行方式，并加入上游 Markdown SVG 转换所需的 `libcairo2`，未恢复高成本递归 chown。
- 上游新增存储审计：上游新增 `PGTableGraphStorage` 后，补齐其 PostgreSQL isolation capability、legacy default alias 和实例级 `storage_profile` 传递。否则 physical knowledge base 会回落到进程级 PostgreSQL 连接配置，且四族 workspace 注册完整性测试会失真；新增/更新的定向回归为 `108 passed`。
- 跨平台兼容修复：sidecar URI 生成改用标准 `Path.as_uri()`，解析使用 `url2pathname()`，兼容 Windows 盘符、空格、Unicode、UNC 语义及旧版本已持久化的 Windows 编码格式；对应回归 `19 passed`。上游 chunk-size 环境变量测试的 storage stub 显式隔离与该测试目标无关的构造期 workspace binding 校验，聚焦回归 `51 passed`。
